...
void SystemInit (void) {
  ...
  uint32_t i;
     ...
     ((USBPLLCLKSEL_Val & 0x03) == 1)   )             /* osc used for SYS/USB PLL   */
  #if ((SYSOSCCTRL_Val & 0x01) == 0)                  /* Oscillator is not bypassed */
    LPC_SYSCON->SYSAHBCLKCTRL |= (1 << 16);           /* Enables IOCON clock        */
    LPC_IOCON->PIO2_0 = ((1 << 0) |                   /* Func = XTALIN              */
                         (0 << 3)  );                 /* no pull-down/pull-up       */
    LPC_IOCON->PIO2_1 = ((1 << 0) |                   /* Func = XTALOUT             */
                         (0 << 3)  );                 /* no pull-down/pull-up       */
  #endif
